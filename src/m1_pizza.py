...
def run_test_draw_points_on_circle():
    ...
    draw_points_on_circle(window, circle, 4, 'maroon')
    window.close_on_mouse_click()
    # -------------------------------------------------------------------------
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    points_list = generate_points_on_circle(circle, number_of_points)
    for k in points_list:
        new_circle = rg.Circle(k, 10)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    points_list = generate_points_on_circle(circle, number_of_slices)
    for k in points_list:
        cut = rg.Line(center, k)
        cut.color = color
...
def run_test_polygon():
    ...
    title = 'Test 3: My Test'
    window = rg.RoseWindow(300, 300, title)
    circle = rg.Circle(rg.Point(150, 150), 100)
    circle.outline_thickness = 2
    polygon(window, circle, 17, 'deep pink', 11)
    window.close_on_mouse_click()
    # -------------------------------------------------------------------------
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points_list) - 1):
        line = rg.Line(points_list[k], points_list[k + 1])
        line.color = color
        line.thickness = thickness
        line.attach_to(window)
    final_line = rg.Line(points_list[0], points_list[len(points_list) - 1])
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(150, 150), 100)
    circle.fill_color = 'deep pink'
    fancy_polygon(window, circle, 51, 25, 'navy', 2)
    window.close_on_mouse_click()
    # -------------------------------------------------------------------------
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(points_list)):
        start = points_list[k]
        points_away = (k + hops_to_next_point) % number_of_lines
        line = rg.Line(start, points_list[points_away])
        line.arrow = 'last'
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
